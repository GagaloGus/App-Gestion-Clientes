...
export class ModalNuevoClienteComponent {
  ...
  private validar(): boolean {
    ...
    if (!this.formData.apellido1?.trim())
      this.errores['apellido1'] = 'El primer apellido es obligatorio.';

    if (this.formData.telefono && !/^\d{9}$/.test(this.formData.telefono.trim()))
      this.errores['telefono'] = 'El teléfono debe tener 9 dígitos.';

    if (this.formData.correo && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(this.formData.correo.trim()))
      this.errores['correo'] = 'El correo no tiene un formato válido.';
